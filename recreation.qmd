...
```{r}
#full_summary <- pmap_dfr(list(models, table_names, model.names), function(model, table_name, model.name){
#  print(class(model.name))
#    }
#  )
```
...
table_names <- list("Collagen (All)", "Collagen (Male)", "Collagen (Female)", "Osteocalcin (All)", "Osteocalcin (Male)", "Osteocalcin (Female)")
model.names <- list("cgam_mf", "cgam_m", "cgam_f", "ogam_mf", "ogam_m", "ogam_f")

get.trends <- function(model, table_name){
  dts <- derivatives(model, select = "s(mean_age)", interval = "simultaneous", partial_match = TRUE) %>% 
    mutate(
      trend = case_when(
        .lower_ci > 0 ~ "increasing",
...
full_summary <- pmap_dfr(inputs, function(model, table_name, model.name){
  s <- summary(model)
  peak <- peak_ages[[model.name]]
  trends <- get.trends(model, table_name)
