...
```{r}
full_summary <- pmap_dfr(list(models, names, model.names), function(model, name, model.name){print(paste0("model s.table= ", summary(model)$s.table[1], "|", "name= ", name, "|", "model.name= ", model.name))
    }
  )
#summary(cgam_mf)
```
...
names <- c("Collagen (All)", "Collagen (Male)", "Collagen (Female)", "Osteocalcin (All)", "Osteocalcin (Male)", "Osteocalcin (Female)")
model.names <- c("cgam_mf", "cgam_m", "cgam_f", "ogam_mf", "ogam_m", "ogam_f")

get.trends <- function(model, name){
  dts <- derivatives(model, select = "s(mean_age)", interval = "simultaneous", partial_match = TRUE) %>% 
    mutate(
      trend = case_when(
        .lower_ci > 0 ~ "increasing",
  ...
  dts <- dts %>% 
    mutate(grp = cumsum(c(TRUE, diff(trend != lag(trend, default = first(trend))) != 0)))
    
  period_list <- dts %>% 
    filter(trend != "nonsig") %>% 
    group_by(trend, grp) %>% 
    summarise(
      start = min(dts$mean_age),
      end = max(dts$mean_age),
      .groups = "drop"
    )
...
full_summary <- pmap_dfr(list(models, names, model.names), function(model, name, model.name){
  s <- summary(model)
  peak <- peak_ages$model.name
  trends <- get.trends(model, name)
  ...
  tibble(
    ...
    p.value = signif(s$s.table[1, "p-value"], 3),
    Peak.Age = round(peak, 2),
    Sig.Increase = paste0(trends %>% filter(trend == "increase")) %>% 
      mutate(interval = paste0(round(start, 2), "-", round(end, 2)) %>% 
      pull(interval), collapse = "; "),
    Sig.Decrease = paste0(trends %>% filter(trend == "decrease")) %>% 
      mutate(interval = paste0(round(start, 2), "-", round(end, 2)) %>% 
      pull(interval), collapse = "; ")
  )
})
