...
```{r}
full_summary <- pmap_dfr(list(models, labs, model.names), function(model, name, model.name){
...
labs <- c("Collagen (All)", "Collagen (Male)", "Collagen (Female)", "Osteocalcin (All)", "Osteocalcin (Male)", "Osteocalcin (Female)")
model.names <- c("cgam_mf", "cgam_m", "cgam_f", "ogam_mf", "ogam_m", "ogam_f")

get.trends <- function(model, lab){
  dts <- derivatives(model, select = "s(mean_age)", interval = "simultaneous", partial_match = TRUE) %>% 
    mutate(
      trend = case_when(
        .lower_ci > 0 ~ "increasing",
...
inputs <- list(models, labs, model.names)

full_summary <- pmap_dfr(inputs, function(model, lab, model.name){
  s <- summary(model)
  peak <- peak_ages[[model.name]]
  trends <- get.trends(model, lab)
