...
models <- list(cgam_mg = cgam_mf, cgam_m = cgam_m, cgam_f = cgam_f, ogam_mf = ogam_mf, ogam_m = ogam_m, ogam_f = ogam_f)
period_list <- imap_dfr(models, function(model, name){
  dts <- derivatives(model, select = "s(mean_age)", interval = "confidence", partial_match = TRUE) %>% 
    mutate(
      sig = case_when(
        .lower_ci > 0 ~ "increasing",
        .upper_ci < 0 ~ "decreasing",
...
Moving on, let's make our segmented regressions
