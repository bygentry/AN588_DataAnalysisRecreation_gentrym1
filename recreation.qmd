...
full_summary <- map2_dfr(models, names, model.names, function(model, name, model.name){
  s <- summary(model)
  ...
  tibble(
    ...
    Sig.Increase = paste(trends %>% filter(trend == "increase")) %>% 
      mutate(interval = paste0(round(start, 2), "-", round(end, 2)) %>% 
      pull(interval), collapse = "; "),
    Sig.Decrease = paste(trends %>% filter(trend == "decrease")) %>% 
      mutate(interval = paste0(round(start, 2), "-", round(end, 2)) %>% 
      pull(interval), collapse = "; ")
  )
})
