...
# both sexes
cgam_mf <- gam(c_conT ~ s(sampleAge, bs = "cr"), method = "REML", data = collag1)

ocgam_mf <- gam(oc_conT ~ s(sampleAge, bs = "cr"), method = "REML", data = osteoc1)

# males only
cgam_m <- gam(c_conT ~ s(sampleAge, bs = "cr"), method = "REML", data = collag1, subset = Sex == "Male")

ocgam_m <- gam(oc_conT ~ s(sampleAge, bs = "cr"), method = "REML", data = osteoc1, subet = Sex == "Male")

# females only
cgam_f <- gam(c_conT ~ s(sampleAge, bs = "cr"), method = "REML", data = collag1, subset = Sex == "Female")

ocgam_f <- gam(oc_conT ~ s(sampleAge, bs = "cr"), method = "REML", data = osteoc1, subset = Sex == "Female")
```
